...
router.get('/book/:id(\\d+)', csrfProtection, 
  asyncHandler(async (req, res) => {
    ...
    const book = await db.Book.findByPk(bookId);
    let bookRating = 'n/a';
    const response = await fetch(`http://host.docker.internal:5000/ratings/${bookId}`);
...
router.get('/book/ratings/:id(\\d+)', csrfProtection,
  asyncHandler(async (req, res) => {
    const bookId = parseInt(req.params.id, 10);
    const rating = {
      value: '',
...
router.post('/book/ratings/:id(\\d+)', csrfProtection,
  asyncHandler(async (req, res) => {
    const bookId = parseInt(req.params.id, 10);
    const {
      value,
    ...
    if (value && email) {
      await fetch(`http://host.docker.internal:5000/ratings/${bookId}?value=${value}&email=${email}`, {
        method: 'POST'
      });
